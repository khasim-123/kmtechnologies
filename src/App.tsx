...
type Particle = {
  x: number; y: number; r: number; c: string; a: number; vx: number; vy: number;
};
...
const GlobalStyles = () => (
  <style>{`
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    /* ── URGENCY BANNER ── */
    .urg-banner{position:fixed;top:0;left:0;right:0;z-index:200;background:linear-gradient(90deg,#1A3C6E 0%,#0f2a52 60%,#1A3C6E 100%);color:#fff;font-size:13px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:20px;padding:9px 16px;text-align:center;flex-wrap:wrap}
    .ub-pill{background:var(--accent);color:#fff;font-size:10px;font-weight:700;padding:3px 10px;border-radius:20px;letter-spacing:.06em;text-transform:uppercase;white-space:nowrap}
    .ub-cta{background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);color:#fff;font-size:11px;font-weight:700;padding:5px 14px;border-radius:20px;cursor:pointer;text-decoration:none;transition:background .2s;white-space:nowrap}
    .ub-cta:hover{background:rgba(255,255,255,0.28)}
    .ub-close{margin-left:8px;background:none;border:none;color:rgba(255,255,255,0.5);font-size:16px;cursor:pointer;line-height:1;padding:0 4px}
    .urg-pulse{width:7px;height:7px;border-radius:50%;background:#22C55E;display:inline-block;margin-right:4px;animation:pulseDot 1.8s infinite}
    body.banner-visible .nav{top:41px}
    body.banner-visible .mobile-menu{top:117px}
    ...
    .nav-inner{width:100%;max-width:none;margin:0;display:flex;align-items:center;justify-content:space-between;height:76px;transition:height .35s;padding:0 5%}
    ...
    /* ── TRUST STRIP ── */
    .trust-strip{background:#fff;border-top:1px solid var(--border);border-bottom:1px solid var(--border);padding:18px 0;overflow:hidden;position:relative;width:100vw;left:50%;margin-left:-50vw;box-sizing:border-box}
    .trust-track{display:flex;gap:52px;align-items:center;animation:scrollTrack 28s linear infinite;width:max-content}
    ...
    .trust-item{display:flex;align-items:center;gap:9px;font-size:13px;font-weight:600;color:var(--muted);white-space:nowrap;opacity:.65;transition:opacity .2s}
    ...
    .process-steps::before{content:'';position:absolute;top:36px;left:12.5%;right:12.5%;height:2px;background:linear-gradient(90deg,var(--primary),var(--accent2),var(--accent),var(--primary));opacity:.2;z-index:0}
    .ps-item{text-align:center;padding:0 16px;position:relative;z-index:1}
    .ps-num{width:72px;height:72px;border-radius:50%;background:#fff;border:2px solid var(--border);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-size:22px;box-shadow:0 4px 16px rgba(0,0,0,0.06);transition:all .3s;position:relative}
    .ps-item:hover .ps-num{background:var(--primary);border-color:var(--primary);transform:scale(1.1)}
    .ps-step-label{font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--accent);margin-bottom:6px}
    .ps-title{font-size:15px;font-weight:700;color:var(--primary);margin-bottom:8px}
    .ps-desc{font-size:12px;color:var(--muted);line-height:1.65}
    ...
    .tech-item{border:1px solid var(--border);border-radius:12px;padding:16px 10px;text-align:center;transition:all .25s;background:var(--bg);cursor:default}
    .tech-item:hover{transform:translateY(-4px);box-shadow:0 12px 28px rgba(0,0,0,0.08);border-color:var(--primary);background:#fff}
    .tech-icon{font-size:28px;margin-bottom:7px}
    .tech-name{font-size:11px;font-weight:600;color:var(--primary)}
    .tech-cat{font-size:9px;color:var(--muted);margin-top:2px;letter-spacing:.05em;text-transform:uppercase}
    ...
    /* ── FREE DEMO CLASS ── */
    #demo{background:linear-gradient(135deg,#EFF6FF 0%,#DBEAFE 40%,#FFF7ED 100%);position:relative;overflow:hidden;width:100vw;left:50%;margin-left:-50vw;box-sizing:border-box;padding:88px 0}
    .demo-inner{display:grid;grid-template-columns:1fr 1fr;gap:64px;align-items:center;padding:0 5%}
    .demo-badge-row{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:18px}
    .demo-badge{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:700;padding:5px 12px;border-radius:20px;letter-spacing:.04em}
    .demo-form-wrap{background:#fff;border-radius:20px;padding:32px;box-shadow:0 24px 64px rgba(26,60,110,0.12)}
    .demo-form-wrap h3{font-size:18px;font-weight:800;color:var(--primary);margin-bottom:5px}
    .demo-form-wrap p{font-size:12px;color:var(--muted);margin-bottom:20px;line-height:1.65}
    .batch-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px}
    .batch-opt{border:1.5px solid var(--border);border-radius:9px;padding:10px 8px;text-align:center;cursor:pointer;transition:all .2s;background:#FAFAF8}
    .batch-opt.selected{border-color:var(--primary);background:#EFF6FF}
    .batch-opt-label{font-size:10px;font-weight:700;color:var(--primary);letter-spacing:.04em;text-transform:uppercase}
    .batch-opt-time{font-size:10px;color:var(--muted);margin-top:2px}
    .emi-note{background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:9px 14px;font-size:11px;color:#166534;font-weight:600;margin-bottom:14px;display:flex;align-items:center;gap:7px}
    .seats-badge{background:#FFF7ED;border:1px solid #FED7AA;border-radius:8px;padding:9px 14px;font-size:11px;color:#92400E;font-weight:600;margin-bottom:14px;display:flex;align-items:center;gap:7px}
    .demo-stats-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:28px}
    .demo-stat-card{background:#fff;border:1px solid var(--border);border-radius:12px;padding:14px 16px}
    .demo-stat-num{font-size:22px;font-weight:800;color:var(--primary);font-family:'Fraunces',serif}
    .demo-stat-label{font-size:11px;color:var(--muted);margin-top:2px}
    .emi-callout{background:#FFF7ED;border:1px solid #FED7AA;border-radius:10px;padding:12px 16px;font-size:13px;color:#92400E;font-weight:600}

    /* ── PLACEMENT STRIP ── */
    .placement-strip{background:var(--primary);padding:28px 0;width:100vw;left:50%;margin-left:-50vw;position:relative;box-sizing:border-box}
    .placement-strip-inner{display:grid;grid-template-columns:repeat(4,1fr);gap:0;text-align:center;padding:0 5%}
    .plst{border-right:1px solid rgba(255,255,255,0.12);padding:0 16px}
    .plst:last-child{border-right:none}
    .plst-big{font-family:'Fraunces',serif;font-size:34px;font-weight:800;color:#fff}
    .plst-label{font-size:11px;color:rgba(255,255,255,0.55);margin-top:3px}
    ...
    .compare-table{width:100%;border-collapse:separate;border-spacing:0;border-radius:16px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.07);margin-top:44px}
    .compare-table th{padding:16px 20px;font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;text-align:center;border-bottom:2px solid var(--border)}
    .compare-table th:first-child{text-align:left}
    .compare-table td{padding:14px 20px;font-size:13px;border-bottom:1px solid var(--border);text-align:center;vertical-align:middle;background:#fff}
    .compare-table td:first-child{text-align:left;font-weight:600;color:var(--primary);background:#FAFAF8}
    .compare-table tr:last-child td{border-bottom:none}
    .col-km{background:#EFF6FF !important}
    .compare-table th.col-km{background:var(--primary);color:#fff}
    .cmp-yes{color:#22C55E;font-size:17px}
    .cmp-no{color:#EF4444;font-size:17px}
    .cmp-partial{color:#F59E0B;font-size:13px;font-weight:600}
    .highlight-row td{background:#FFFBEB}
    .highlight-row td.col-km{background:#EFF6FF !important}
    ...
    .faq-q{padding:18px 22px;font-size:14px;font-weight:600;color:var(--primary);cursor:pointer;display:flex;justify-content:space-between;align-items:center;user-select:none;background:none;border:none;width:100%;text-align:left;font-family:'Plus Jakarta Sans',sans-serif}
    ...
    @media(max-width:900px){
      .hero-inner,.about-team-row,.contact-grid,.demo-inner{grid-template-columns:1fr}
      ...
      .placement-strip-inner{grid-template-columns:repeat(2,1fr)}
      .plst:nth-child(2){border-right:none}
      .plst:nth-child(3){border-top:1px solid rgba(255,255,255,0.12)}
      .plst:nth-child(4){border-top:1px solid rgba(255,255,255,0.12);border-right:none}
...
/* ─────────────────────────────────────────────
   KM LOGO (dark bg / footer)
───────────────────────────────────────────── */
const KMLogoFooter = ({ size = 52 }) => (
  <img
    src={logoImage}
    alt="KM Technologies logo"
    width={size}
    height={size}
...
function useParticleCanvas(canvasRef: RefObject<HTMLCanvasElement | null>) {
  useEffect(() => {
    ...
    const mkP = (): Particle => ({ x:Math.random()*w, y:Math.random()*h, r:Math.random()*2+1, c:COLORS[Math.floor(Math.random()*3)], a:Math.random()*.4+.1, vx:(Math.random()-.5)*.35, vy:(Math.random()-.5)*.35 });
    ...
    const loop = () => {
      ...
      for(let i=0;i<particles.length;i++){
        for(let j=i+1;j<particles.length;j++){
          ...
          if(d<110){ ctx.beginPath(); ctx.strokeStyle=`rgba(26,60,110,${0.07*(1-d/110)})`; ctx.lineWidth=.5; ctx.moveTo(particles[i].x,particles[i].y); ctx.lineTo(particles[j].x,particles[j].y); ctx.stroke(); }
        }
      }
...
function UrgencyBanner() {
  ...
  return (
    <div className="urg-banner">
      <span className="ub-pill">🎓 New Batch</span>
      <span>
        <span className="urg-pulse"></span>{' '}
        .NET Full Stack batch starting <strong>April 28</strong> · Only <strong style={{color:"#FCD34D"}}>6 seats left</strong>
      </span>
      <a href="#demo" className="ub-cta" onClick={e=>{e.preventDefault();setVisible(false);setTimeout(()=>scrollTo("#demo"),100)}}>Book Free Demo →</a>
      <button className="ub-close" onClick={()=>setVisible(false)} aria-label="Close">×</button>
    </div>
  );
}
...
function Nav() {
  ...
  const go=(e:MouseEvent<HTMLAnchorElement>,id:string)=>{ e.preventDefault(); setOpen(false); scrollTo(id); };
  const navIds=["#about","#services","#portfolio","#training","#pricing","#college","#contact"];
  return (
    <>
      <nav className={`nav${scrolled?" scrolled":""}`}>
        <div className="nav-inner" style={{padding:"45px"}}>
          <a className="nav-logo" href="#hero" onClick={e=>{e.preventDefault();scrollTo("#hero")}}>
            <div className="logo-mark" style={{
    borderRadius: "50px",
    overflow: "hidden",   // 👈 IMPORTANT
    marginTop: "5px",
  }}><img src={logoImage} alt="KM Technologies logo" style={{width:"100%",height:"100%",objectFit:"cover"}}/></div>
          ...
          <ul className="nav-links">
            {navIds.map(id=>(
              <li key={id}><a href={id} onClick={e=>go(e,id)}>{id.replace("#","").charAt(0).toUpperCase()+id.slice(2)}</a></li>
            ))}
          </ul>
      ...
      <div className={`mobile-menu${open?" open":""}`}>
        {navIds.map(id=>(
          <a key={id} href={id} onClick={e=>go(e,id)}>{id.replace("#","").charAt(0).toUpperCase()+id.slice(2)}</a>
        ))}
      </div>
    </>
  );
      ...
      <div className="hero-inner">
        <div className="hero-content fade-in">
          ...
          <div className="hero-btns">
            <a href="#contact" className="btn-primary" onClick={e=>{e.preventDefault();scrollTo("#contact")}}>Start a Project ↗</a>
...
───────────────────────────────────────────── */
const TRUST_ITEMS = [
  "⚖️ Legal Tech Platforms","🌾 AgriTech Marketplaces","👥 HR & Payroll Systems",
  "🏢 Enterprise Web Apps","📱 Android & iOS Apps","☁️ Azure Cloud Deployments",
  "🎓 500+ Students Placed","🏆 Top Performer 2025","🔒 NDA on Every Project","✅ CI/CD 99.9% Uptime",
];
function TrustStrip() {
  ...
  return (
    <div className="trust-strip">
      <div className="trust-track">
        {doubled.map((item,i)=>(
          <div key={`${item}-${i}`} className="trust-item">
...
const IMGS = {
  ...
  business:"https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=600&q=80",
  students:"https://images.unsplash.com/photo-1523240795612-9a054b0db644?w=600&q=80",
  college:"https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=600&q=80",
  dev1:"https://images.unsplash.com/photo-1607990281513-2c110a25bd8c?w=200&q=80",
  dev2:"https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?w=200&q=80",
  dev3:"https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=200&q=80",
};
function About() {
  return (
    <section id="about">
      <div className="sec-inner">
        ...
        <div className="about-team-stats fade-in">
          {[["10+","Years","Combined developer experience"],["15+","Experts","Full-stack devs, QA & cloud engineers"],["5+","Trainers","Industry veterans, real enterprise systems"],["500+","Students","Placed in top IT companies"]].map(([num,unit,label])=>(
        ...
        <div className="fade-in" style={{marginBottom:52}}>
          <div style={{borderRadius:16,overflow:"hidden",position:"relative",height:220,boxShadow:"0 12px 40px rgba(0,0,0,0.1)"}}>
            <img src={IMGS.team} alt="KM Technologies team" style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 30%"}}/>
            ...
            <div style={{position:"absolute",top:"50%",left:36,transform:"translateY(-50%)"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {[IMGS.dev1,IMGS.dev2,IMGS.dev3].map((src,i)=>(
                  <img key={src} src={src} alt="Team member" style={{width:40,height:40,borderRadius:"50%",objectFit:"cover",border:"2px solid rgba(255,255,255,0.8)",marginLeft:i===0?0:-10}}/>
                ))}
        ...
        <div className="about-audience fade-in">
          {[
            {cls:"aa-biz",src:IMGS.business,title:"For Businesses & Startups",overlay:"rgba(26,60,110,0.7)",check:{bg:"#EFF6FF",color:"#1D4ED8"},desc:"You get a battle-tested team — not a solo freelancer. Our developers and testers have shipped enterprise-grade systems with proper QA, code reviews and Azure deployment.",points:["Dedicated developer + QA tester per project","Azure cloud deployment with CI/CD pipelines","Post-delivery support up to 3 months","NDA and IP protection on every engagement"]},
            {cls:"aa-stu",src:IMGS.students,title:"For Students & Job Seekers",overlay:"rgba(194,65,12,0.7)",check:{bg:"#FFF7ED",color:"#C2410C"},desc:"Learn from developers active in production codebases — not outdated tutorials. Our trainers have 10+ years of real industry experience across .NET, Java, Python and Go.",points:["Trainers with 10+ years enterprise experience","Build real projects — not toy apps","Certificate issued on official letterhead","Placement mentorship until you land your first job"]},
            {cls:"aa-col",src:IMGS.college,title:"For Colleges & Institutions",overlay:"rgba(109,40,217,0.7)",check:{bg:"#F5F3FF",color:"#6D28D9"},desc:"Partner with a team that understands both academia and industry. We've mentored 500+ students and conducted workshops across engineering colleges in Hyderabad.",points:["Workshops by industry practitioners, not lecturers","Live project deliverables during the session","College MOU partnerships with long-term programmes","Guest lectures, hackathons and internship support"]},
          ].map(c=>(
            <div key={c.title} className={`aa-card ${c.cls}`}>
              <div style={{height:140,borderRadius:10,overflow:"hidden",marginBottom:18,position:"relative"}}>
                <img src={c.src} alt={c.title} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                <div style={{position:"absolute",inset:0,background:`linear-gradient(180deg,transparent 40%,${c.overlay} 100%)`}}/>
                <div style={{position:"absolute",bottom:10,left:12,color:"#fff",fontSize:13,fontWeight:700}}>{c.title}</div>
              </div>
              <p style={{fontSize:13,color:"var(--muted)",lineHeight:1.75,marginBottom:14}}>{c.desc}</p>
              <div className="aa-points">
                {c.points.map(pt=>(
                  <div key={pt} className="aa-point">
                    <div className="aa-check" style={{background:c.check.bg,color:c.check.color}}>✓</div>
                    <span>{pt}</span>
                  </div>
                ))}
              </div>
        ...
        <div className="about-team-row fade-in">
          <div>
            <div style={{borderRadius:24,overflow:"hidden",position:"relative",maxWidth:380,margin:"0 auto",boxShadow:"0 24px 64px rgba(26,60,110,0.18)"}}>
              ...
              <div style={{position:"absolute",top:16,right:16,background:"rgba(255,255,255,0.95)",borderRadius:10,padding:"6px 10px",display:"flex",alignItems:"center",gap:6,boxShadow:"0 4px 14px rgba(0,0,0,0.15)"}}>
                ...
                <div><div style={{fontSize:9,fontWeight:700,color:"var(--primary)",letterSpacing:"0.06em",textTransform:"uppercase"}}>Microsoft</div><div style={{fontSize:10,fontWeight:600,color:"var(--muted)"}}>AZ-900 Certified</div></div>
              </div>
            </div>
          </div>
          <div className="atr-content">
            ...
            <p>KM Technologies is led by Khasim Meeravali — a Full Stack .NET Developer with hands-on enterprise experience building legal tech platforms, agri-commerce marketplaces and HR systems for real businesses.</p>
            <p>Unlike agencies that assign junior interns, every client gets direct access to senior developers and testers who have solved these problems in production environments.</p>
...
───────────────────────────────────────────── */
const STEPS=[
  {emoji:"💬",label:"Step 1",title:"Free Consultation",desc:"Tell us your idea. We listen, ask the right questions and understand your goals — no tech jargon, no pressure."},
  {emoji:"📋",label:"Step 2",title:"Scope & Timeline",desc:"We send you a clear project plan, technology recommendation, timeline and honest pricing. No surprises."},
  {emoji:"⚡",label:"Step 3",title:"Build & Review",desc:"Your dedicated developer and QA tester build the product. You review progress weekly with a live demo link."},
  {emoji:"🚀",label:"Step 4",title:"Launch & Support",desc:"We deploy to Azure and hand over everything. Then we stay — free support included for up to 3 months."},
];
    ...
    <section id="process">
      <div className="sec-inner">
        <div className="sec-head fade-in" style={{textAlign:"center"}}>
          <div className="sec-label" style={{textAlign:"center"}}>How We Work</div>
          <h2 className="sec-title" style={{maxWidth:580,margin:"0 auto 14px"}}>From first call to launch — a clear process</h2>
          <p className="sec-sub" style={{margin:"0 auto",textAlign:"center"}}>No guesswork. No long silences. Every step is transparent so you always know what's happening with your project.</p>
        </div>
        <div className="process-steps">
          {STEPS.map(s=>(
            <div key={s.label} className="ps-item fade-in">
              <div className="ps-num"><span>{s.emoji}</span></div>
...
───────────────────────────────────────────── */
const TECHS=[
  {icon:"⚡",name:"ASP.NET Core",cat:"Backend"},{icon:"⚛️",name:"React.js",cat:"Frontend"},
  {icon:"📱",name:"React Native",cat:"Mobile"},{icon:"🐍",name:"Python",cat:"Backend"},
  {icon:"☕",name:"Java",cat:"Backend"},{icon:"🔵",name:"Go",cat:"Backend"},
  {icon:"🗄️",name:"SQL Server",cat:"Database"},{icon:"🐘",name:"PostgreSQL",cat:"Database"},
  {icon:"☁️",name:"Azure",cat:"Cloud"},{icon:"🐳",name:"Docker",cat:"DevOps"},
  {icon:"🔧",name:"CI/CD",cat:"DevOps"},{icon:"🅰️",name:"Angular",cat:"Frontend"},
  {icon:"🔑",name:"JWT Auth",cat:"Security"},{icon:"📡",name:"REST APIs",cat:"Integration"},
  {icon:"⚙️",name:"gRPC",cat:"Integration"},{icon:"📊",name:"EF Core",cat:"ORM"},
  {icon:"🌐",name:"Next.js",cat:"Frontend"},{icon:"🤖",name:"AI / LLM",cat:"AI"},
];
function TechStack() {
  return (
    <section id="techstack" style={{background:"#fff"}}>
      <div className="sec-inner">
        <div className="sec-head fade-in" style={{textAlign:"center"}}>
          <div className="sec-label" style={{textAlign:"center"}}>Technologies</div>
          <h2 className="sec-title" style={{maxWidth:540,margin:"0 auto 14px"}}>The full stack — from database to deployment</h2>
          <p className="sec-sub" style={{margin:"0 auto",textAlign:"center"}}>We work across the complete modern tech stack so you never need to hire multiple vendors.</p>
        </div>
...
───────────────────────────────────────────── */
const BATCHES=[
  {label:"Weekday",time:"Mon–Fri · 7 AM or 7 PM"},
  {label:"Weekend",time:"Sat–Sun · 9 AM or 2 PM"},
  {label:"Online",time:"Zoom · Flexible timing"},
];
const DEMO_COURSES=[".NET Full Stack","Python Full Stack","Java Full Stack","Go Programming"];

function DemoClassSection() {
  const [selectedBatch,setSelectedBatch]=useState(0);
  const [form,setForm]=useState({name:"",phone:"",course:".NET Full Stack"});
  const [sent,setSent]=useState(false);
  const submit=()=>{
    if(!form.name||!form.phone){alert("Please enter your name and phone number.");return;}
    const text=`Hi, I want to book a FREE DEMO class.%0AName: ${form.name}%0APhone: ${form.phone}%0ACourse: ${form.course}%0ABatch: ${BATCHES[selectedBatch].label} (${BATCHES[selectedBatch].time})`;
    window.open(`https://wa.me/919346819082?text=${text}`,"_blank");
    setSent(true);
    setForm({name:"",phone:"",course:".NET Full Stack"});
  };
  return (
    <section id="demo">
      <div className="demo-inner">
        <div className="fade-in">
          <div className="demo-badge-row">
            <span className="demo-badge" style={{background:"#F0FDF4",color:"#166534"}}>✅ 100% Free — No obligations</span>
            <span className="demo-badge" style={{background:"#EFF6FF",color:"#1D4ED8"}}>🎓 By Industry Experts</span>
          </div>
          <div className="sec-label">Free Demo Class</div>
          <h2 className="sec-title" style={{fontSize:34,marginBottom:14}}>Try before you pay.<br/>Book a free demo class.</h2>
          <p style={{fontSize:15,color:"var(--muted)",lineHeight:1.8,marginBottom:28}}>Not sure if this is the right course for you? Join one free demo class — no payment, no pressure. Experience our teaching style and ask any questions before you enrol.</p>
          <div className="demo-stats-grid">
            {[["500+","Students placed in IT companies"],["₹3–8 LPA","Average starting salary after training"],["10+","Years of combined trainer experience"],["4 Stacks",".NET · Python · Java · Go"]].map(([num,label])=>(
              <div key={label} className="demo-stat-card">
                <div className="demo-stat-num">{num}</div>
                <div className="demo-stat-label">{label}</div>
              </div>
            ))}
          </div>
          <div className="emi-callout">
            💳 Easy EMI available — start your course from as low as <strong>₹2,500/month</strong>. No interest, no hidden charges.
          </div>
        </div>
        <div className="fade-in">
          <div className="demo-form-wrap">
            <h3>Book Your Free Demo</h3>
            <p>Select your preferred batch time and course — we'll confirm within 2 hours on WhatsApp.</p>
            <div className="seats-badge">🔥 <span>Next batch: <strong>April 28</strong> · Only <strong>6 seats</strong> remaining</span></div>
            <div style={{fontSize:11,fontWeight:700,color:"var(--primary)",letterSpacing:".05em",textTransform:"uppercase",marginBottom:8}}>Preferred Batch</div>
            <div className="batch-grid">
              {BATCHES.map((b,i)=>(
                <button key={b.label} type="button" className={`batch-opt${selectedBatch===i?" selected":""}`} onClick={()=>setSelectedBatch(i)}>
                  <div className="batch-opt-label">{b.label}</div>
                  <div className="batch-opt-time">{b.time}</div>
                </button>
              ))}
            </div>
            <div className="form-group">
              <label htmlFor="demo-course">Course</label>
              <select id="demo-course" value={form.course} onChange={e=>setForm({...form,course:e.target.value})}>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input id="demo-name" type="text" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Full name"/>
              </div>
              <div className="form-group">
                <label htmlFor="demo-phone">WhatsApp Number</label>
                <input id="demo-phone" type="tel" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="+91 XXXXX XXXXX"/>
              </div>
            </div>
            <div className="emi-note">💳 EMI from ₹2,500/month · 0% interest · Pay after demo</div>
            <button className="form-submit" onClick={submit}>📲 Book Free Demo on WhatsApp</button>
            {sent&&<div className="form-success">✓ Request sent! We'll confirm your demo slot within 2 hours.</div>}
            <p style={{fontSize:11,color:"var(--muted)",textAlign:"center",marginTop:12}}>No spam. No calls without permission. Just a class.</p>
          </div>
        </div>
      </div>
    </section>
...
function PlacementStrip() {
  return (
    <div className="placement-strip">
      <div className="placement-strip-inner">
        {[["500+","Students Trained & Placed"],["₹3–8 LPA","Avg. Starting Package"],["2–3 Months","Avg. Time to First Job"],["4.9 ⭐","Average Student Rating"]].map(([big,label])=>(
          <div key={label} className="plst">
            <div className="plst-big">{big}</div>
            <div className="plst-label">{label}</div>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="pricing-grid">
          {cards.map(c=>(
            <div key={c.name} className={`price-card fade-in${c.featured?" featured":""}`}>
              ...
              <a href="#contact" className={`pc-btn ${c.featured?"pc-btn-primary":"pc-btn-outline"}`} onClick={e=>{e.preventDefault();scrollTo("#contact")}}>Get Quote</a>
            </div>
          ))}
        </div>
      </div>
...
───────────────────────────────────────────── */
const COMPARE_ROWS=[
  {feature:"Industry-experienced trainers",km:true,institute:"partial",freelancer:false},
  {feature:"Real enterprise project during training",km:true,institute:false,freelancer:false},
  {feature:"Certificate on official letterhead",km:true,institute:true,freelancer:false},
  {feature:"Placement mentorship until employed",km:true,institute:"partial",freelancer:false},
  {feature:"EMI / pay-in-parts option",km:true,institute:true,freelancer:false},
  {feature:"1:1 doubt clearing sessions",km:true,institute:false,freelancer:true},
  {feature:"Weekday + Weekend + Online batches",km:true,institute:"partial",freelancer:"partial"},
  {feature:"Agency projects to build portfolio",km:true,institute:false,freelancer:false},
  {feature:"Post-course career support",km:true,institute:false,freelancer:false},
];
type CmpValProps = Readonly<{ val: boolean | string }>;
function CmpVal({ val }: CmpValProps) {
  if (val === true) return <span className="cmp-yes">✓</span>;
  if (val === false) return <span className="cmp-no">✗</span>;
  return <span className="cmp-partial">Partial</span>;
}
function ComparisonTable() {
  return (
    <section id="compare" style={{background:"var(--bg)"}}>
      <div className="sec-inner">
        <div className="sec-head fade-in" style={{textAlign:"center"}}>
          <div className="sec-label" style={{textAlign:"center"}}>Why KM Technologies</div>
          <h2 className="sec-title" style={{maxWidth:560,margin:"0 auto 14px"}}>How we compare to the alternatives</h2>
          <p className="sec-sub" style={{margin:"0 auto",textAlign:"center"}}>See exactly what you get with KM Technologies versus a typical Hyderabad training institute or a solo freelance trainer.</p>
        </div>
        <div className="fade-in" style={{overflowX:"auto"}}>
          <table className="compare-table">
            <thead>
              <tr>
                <th style={{background:"#FAFAF8",textAlign:"left",color:"var(--muted)"}}>Feature</th>
                <th className="col-km">🏆 KM Technologies</th>
                <th style={{background:"#fff",color:"var(--muted)"}}>Big Institute</th>
                <th style={{background:"#fff",color:"var(--muted)"}}>Solo Freelancer</th>
              </tr>
            </thead>
            <tbody>
              {COMPARE_ROWS.map((row,i)=>(
                <tr key={row.feature} className={i%2===1?"highlight-row":""}>
                  <td>{row.feature}</td>
                  <td className="col-km"><CmpVal val={row.km}/></td>
                  <td><CmpVal val={row.institute}/></td>
                  <td><CmpVal val={row.freelancer}/></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={{textAlign:"center",marginTop:32}}>
          <a href="#demo" className="btn-primary" onClick={e=>{e.preventDefault();scrollTo("#demo")}}>Book Your Free Demo Class →</a>
        </div>
      </div>
    </section>
  );
    ...
    <section id="why">
      <div className="sec-inner">
        ...
        <div className="why-grid">
          {[["10+","Years Experience","Combined developer experience across our team"],["15+","Expert Team","Senior developers and QA testers who've shipped enterprise systems"],["500+","Students Trained","Placed in top IT companies through hands-on training"],["∞","Post-Delivery Support","We don't disappear after go-live. Ongoing support included"]].map(([n,t,d])=>(
...
function Faq() {
  ...
  const faqs=[
    ...
    ["Is the demo class really free?","100% free — no payment, no conditions, no pressure. You attend the demo, experience our teaching style, ask questions, and then decide if you want to enrol. No obligation whatsoever."],
    ["Are there EMI options for training courses?","Yes. We offer flexible payment plans starting from ₹2,500/month with 0% interest. You can discuss your preferred payment structure after attending the free demo class."],
  ];
  return (
    <section id="faq">
      <div className="sec-inner">
        ...
        <div className="contact-grid">
          ...
          <div className="fade-in">
            <div className="contact-form">
              <div className="form-row">
                <div className="form-group"><label htmlFor="c-name">Your Name</label><input id="c-name" type="text" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Enter your name"/></div>
                <div className="form-group"><label htmlFor="c-phone">Phone / WhatsApp</label><input id="c-phone" type="tel" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="Your number"/></div>
              </div>
              <div className="form-group"><label htmlFor="c-email">Email</label><input id="c-email" type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} placeholder="your@email.com"/></div>
              <div className="form-group">
              ...
              <div className="form-group"><label htmlFor="c-msg">Tell us what you need</label><textarea id="c-msg" value={form.msg} onChange={e=>setForm({...form,msg:e.target.value})} placeholder="Describe your requirement briefly..."/></div>
      ...
      <div className="footer-inner">
        <div className="footer-top">
          ...
          <div className="footer-col">
            <h4>Services</h4>
            <a href="#services" onClick={e=>{e.preventDefault();scrollTo("#services")}}>Web Development</a>
            <a href="#services" onClick={e=>{e.preventDefault();scrollTo("#services")}}>Mobile Apps</a>
            <a href="#training" onClick={e=>{e.preventDefault();scrollTo("#training")}}>Training</a>
            <a href="#college" onClick={e=>{e.preventDefault();scrollTo("#college")}}>College Workshops</a>
          </div>
          <div className="footer-col">
            <h4>Training</h4>
            <a href="#training" onClick={e=>{e.preventDefault();scrollTo("#training")}}>.NET Full Stack</a>
            <a href="#training" onClick={e=>{e.preventDefault();scrollTo("#training")}}>Python Full Stack</a>
            <a href="#training" onClick={e=>{e.preventDefault();scrollTo("#training")}}>Java Full Stack</a>
            <a href="#training" onClick={e=>{e.preventDefault();scrollTo("#training")}}>Go Programming</a>
          </div>
          <div className="footer-col">
            <h4>Contact</h4>
            ...
            <span style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>Hyderabad, Telangana</span>
          </div>
        </div>
